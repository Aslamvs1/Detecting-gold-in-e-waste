...
# --- CONFIGURATION ---
BINARY_THRESHOLD = 100      # Adjust 80-120 if box flickers
MODEL_PATH = 'models/best_gold_model_v2.keras'
CONFIDENCE_THRESHOLD = 0.50 # Standard threshold
GOLD_IS_ZERO = False        # Change to True if labels are swapped
CAMERA_INDEX = 1            # 0 or 1 for DroidCam
...
while True:
    ...
    _, thresh = cv2.threshold(blur, BINARY_THRESHOLD, 255, cv2.THRESH_BINARY_INV)
    ...
    if largest_contour is not None:
        ...
        if roi.size > 0:
            try:
                ...
                is_gold = False
                if GOLD_IS_ZERO:
                    if score < CONFIDENCE_THRESHOLD: is_gold = True
                ...
                # --- FIX: FORCE TEXT VISIBILITY ---
                # Ensure text doesn't go off-screen
                text_y = y - 10 if y - 10 > 25 else y + h + 25

                # Draw Black Background Box for Text
                (text_w, text_h), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)
                cv2.rectangle(frame, (x, text_y - text_h - 5), (x + text_w, text_y + 5), (0,0,0), -1)

                # Draw Text
                cv2.putText(frame, label, (x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
